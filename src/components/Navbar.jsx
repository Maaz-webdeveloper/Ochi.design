...
const Navbar = () => {
  ...
  useEffect(() => {
    const controlNavbar = () => {
      if (typeof window !== 'undefined') {
        // Niche scroll karne par hide ho jaye (agar 50px se ziada scroll ho chuka ho)
        if (window.scrollY > lastScrollY && window.scrollY > 50) {
          setShow(false);
        } else {
          // Uper scroll karne par show ho jaye
          setShow(true);
        }
  ...
  return (
    <div
      className={`w-full px-5 md:px-20 py-5 md:py-8 flex items-center justify-between fixed z-[999] top-0 left-0 
      backdrop-blur-md bg-white/10  transition-transform duration-500 ease-in-out
    ...
    >
      ...
      {/* Desktop Links */}
      <div className="links font-neue hidden md:flex gap-10">
        {["Services", "Our Work", "About us", "Insights", "Contact us"].map((item, index) => {
          return (
            <a
              key={index}
              className={`capitalize text-lg font-light cursor-pointer ${index === 4 && "ml-20 lg:ml-32"}`}
            >
              {item}
            </a>
          )
      ...
      <div className="md:hidden">
        <button className="text-sm uppercase tracking-wider font-light border border-white/50 px-4 py-1.5 rounded-full">
          Menu
        </button>
      </div>
